...
export default function QFS() {
  ...
  const getRewardPool = async () => {
    ...
    const pendingPayout =
      parseFloat(post.pending_payout_value.split(" ")[0]) / 2;
  ...
  return (
    ...
    >
      {userStats.username !== "guest" && isLoggedIn() ? (
        <Flex direction="row" align="center" gap={10} marginBottom={10}>
          <Flex
            direction="row"
            alignItems="center"
            ...
            paddingRight={2}
          >
            <Image src="assets/coinspin.gif" width="30px" height="30px" />

            <Text fontSize="2xl">{userStats.highscore.toFixed(0)}</Text>
          </Flex>
          ...
          <Flex
            ...
            paddingRight={2}
          >
            <Image src="assets/clockspin.gif" width="28px" height="28px" />

            <Text fontSize="2xl">
              {userStats.time > 60
                ? `${Math.floor(userStats.time / 60)}m ${userStats.time % 60}s`
                : `${userStats.time}s`}
            </Text>
          </Flex>
        </Flex>
      ...
      <Box padding={2} marginBottom={10}>
        <iframe
          width="1280"
          height="720"
          style={{ background: "#000000", borderRadius: "20px" }}
        />
      </Box>
      ...
      <Text fontSize="3xl">
        <Link
          ...
          href={rewardPool?.link || "https://peakd.com/@stoken.quest/"}
          isExternal
        >
      ...
      >
        ...
        <Text fontSize="2xl">Reward Pool: ${rewardPool?.reward}</Text>
      </Flex>
          ...
          <Table width="100%" border="1px solid green">
            <Thead>
              <Tr>
                ...
                <Th border="0px" w="20%" textAlign="center">
                  Highscore
                </Th>{" "}
                {/* Set width and center align */}
              </Tr>
            </Thead>
            <Tbody>
              {leaderboard.map((user, index) => (
                <Tr
                  key={index}
                  bg={
                    (index + 1) % 2 === 0 ? "black" : "rgba(83, 206, 63, 0.3)"
                  }
                  color="white"
                >
                  {" "}
                  ...
                  <Td border="0px">
                    ...
                    <Link
                      style={{ textDecoration: "none" }}
                      href={`https://skatehive.app/profile/${user.username}`}
                      isExternal
                    >
                      @{user.username}
                    </Link>
                  </Td>
                  <Td isNumeric textAlign="center" border="0px">
                    {" "}
                    {/* Set background color for the cell */}
          ...
          >
            <Thead>
              <Tr>
                ...
                <Th border="none" w="20%" textAlign="center">
                  Time
                </Th>{" "}
                {/* Set width and center align */}
              </Tr>
            ...
            <Tbody>
              {bestTimes.map((user, index) => (
                ...
                >
                  ...
                  <Td isNumeric textAlign="center" border="none">
                    {" "}
                    {/* Set background color for the cell */}
                    {user.time}
                  </Td>
