...
const PostModal: React.FC<Types.PostModalProps> = ({
  ...
  const handleSaveClick = () => {
    ...
    if (username && window.hive_keychain) {
      ...
      console.error(
        "Hive Keychain extension not found or user not authenticated!"
      );
    }
  ...
  const handleVote = async () => {
    if (!user || !user.name) {
      // Use user.name to get the username
  ...
  return (
    <Modal isOpen={isOpen} onClose={onClose} size="3xl">
      ...
      <ModalContent
        ...
        boxShadow="0px 0px 10px 5px rgba(128,128,128,0.1)"
      >
        <ModalHeader>
          <PostHeader
            title={title}
        ...
        {isEditing ? (
          <center>
            ...
            >
              <Text fontSize="42px">Select New Thumbnail</Text>
            </Tooltip>
          </center>
        ) : null}
        ...
        <ModalBody ref={modalContainerRef}>
          {isEditing ? (
            <VStack>
              <Text fontSize="42px">Change your Post</Text>
              <Textarea
                value={editedContent}
