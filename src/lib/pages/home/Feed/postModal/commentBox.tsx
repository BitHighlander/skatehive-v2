...
const CommentBox: React.FC<Types.CommentBoxProps> = ({
  user,
  parentAuthor,
  parentPermlink,
  onCommentPosted,
      ...
      if (parentAuthor) {
        try {
          ...
          if (Array.isArray(response) && response.length > 0) {
            const authorMetadata = JSON.parse(
              response[0].json_metadata
            ).extensions;
  ...
  const sendHiveTipModal = () => {
    ...
    return (
      <Modal
        isOpen={isSendHiveTipModalOpen}
        onClose={() => setSendHiveTipModalOpen(false)}
      >
        <ModalContent minW="30%" bg="black" border="3px solid red">
          <ModalCloseButton color="red" />
          <ModalHeader>Send Hive Tip to {parentAuthor}</ModalHeader>
          <center>
            <Image boxSize="148px" src="assets/santapepe.png" />
          </center>
          ...
          <Box p="10px" textAlign="center">
            ...
            <Box p="10px" textAlign="center">
              <Text textAlign="center" color="white">
                Send a Message with Transaction
              </Text>
              <Input
                margin="2px"
            ...
            <Button
              ...
              marginLeft="10px"
              onClick={() =>
                sendHive(commentContent, parentAuthor || "", defaultMemo)
              }
            >
              Tip {commentContent} HIVE
            </Button>
          </Box>
        </ModalContent>
      </Modal>
  ...
  const sendEthTipToast = () => {
    ...
    const title = "Ethereum Tip";
    let description = "";
    let duration = 3000;

    if (!isEthAddressPresent) {
      description =
        "This user has not set an Ethereum address yet. Don´t let it happen with you, close this to add yours!";
      status = "error";
      duration = 10000;
    } else {
  ...
  return (
    ...
    >
      ...
      {isHiveLoginModalOpen && (
        <HiveLogin isOpen onClose={() => setHiveLoginModalOpen(false)} />
      )}
    </Box>
  );
};
