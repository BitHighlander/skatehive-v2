...
const Comment: React.FC<Types.CommentProps> = ({
  author,
  body,
  created,
  net_votes,
        ...
        <ReactMarkdown
          ...
          components={{
            ...
            ol: ({ node, children, ...props }) => (
              <ol {...props} style={{ paddingLeft: "20px" }}>
                {children}
              </ol>
            ),
            ul: ({ node, children, ...props }) => (
              <ul {...props} style={{ paddingLeft: "20px" }}>
                {children}
              </ul>
            ),
          }}
...
const Comments: React.FC<Types.CommentsProps> = ({
  comments,
  commentPosted,
  blockedUser,
}) => {
  const [localComments, setLocalComments] =
    useState<Types.CommentProps[]>(comments);
  const blockedUsersList = Array.isArray(blockedUser)
    ? blockedUser
    : [blockedUser]; // Convert to an array if it's not already
  ...
  const filterBlockedUserComments = (
    comments: Types.CommentProps[],
    blockedUsers: string[]
  ): Types.CommentProps[] => {
    return comments.filter(
      (comment: Types.CommentProps) => !blockedUsers.includes(comment.author)
    );
  };

  // Filter comments based on blockedUser
  const filteredComments = filterBlockedUserComments(
    localComments,
    blockedUsersList
  );
