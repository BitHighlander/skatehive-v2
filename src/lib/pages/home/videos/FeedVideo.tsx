...
const adjustVideoSize = (iframe: string): string => {
  ...
  if (
    ...
    odyseeDomains.some((domain) => iframe.includes(domain))
  ) {
    iframe = iframe
      .replace(/width\s*=\s*"\d+"/, "")
      .replace(/height\s*=\s*"\d+"/, "")
      .replace(
        "<iframe",
        '<iframe style="position: absolute; top: 0; left: 0; width: 100%; height: 100%;" allow="accelerometer; encrypted-media; gyroscope; picture-in-picture" allowfullscreen allow-scripts'
      );
    return iframe;
  }
...
const HiveVideos: React.FC = () => {
  ...
  const extractIframes = (markdownContent: string): string[] => {
    ...
    const nonPinataIframes = matches.filter(
      (iframe) => !iframe.includes("gateway.pinata.cloud")
    );
    ...
    <div style={{ width: "100%", margin: "0 auto" }}>
      <SimpleGrid columns={{ base: 1, md: 3 }} spacing={5}>
        {posts.map((post, index) => {
          const modifiedContent = transform3SpeakContent(post.body);
          const iframes = extractIframes(modifiedContent);
          return iframes.length > 0
            ? iframes.map((iframe, i) => (
                <Box key={`${index}-${i}`} borderRadius="md" m={2} p={2}>
                  ...
                  <Box>
                    ...
                    >
                      Full Post |<ChakraText ml={2}>{post.author}</ChakraText>
                      <Avatar
