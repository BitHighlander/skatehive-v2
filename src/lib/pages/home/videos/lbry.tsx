...
const VhsTape: React.FC<VhsTapeProps> = ({
  ...
  return (
    <Box style={tapeStyle} onClick={onSelect} bg="blue">
      <img src={imageUrl} alt={title} style={imageStyle} />
      <label style={{ fontWeight: "bold", marginTop: "10px", color: "black" }}>
        {title}
      </label>
    </Box>
  );
...
const Shelf = () => {
  ...
  const [selectedVideoSoundtrack, setSelectedVideoSoundtrack] = useState<Record<
    string,
    string[]
  > | null>(null);
  ...
  return (
    <Flex minW="100%" height="100%">
      ...
      <Box>
        <Center>
          {selectedVideoUrl && (
            <Box border="3px solid limegreen" borderRadius="15px">
              <iframe
                ...
                {Object.keys(selectedVideoSoundtrack).map((section) => (
                  <Tr key={section}>
                    ...
                    <Td maxW="300px" isTruncated>
                      {selectedVideoSoundtrack[section].join(", ")}
                    </Td>
                  </Tr>
                ))}
              </Tbody>
