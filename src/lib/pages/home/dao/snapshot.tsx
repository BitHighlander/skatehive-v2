...
const SkatehiveProposals: React.FC = () => {
  ...
  const getSummary = async (body: string) => {
    ...
    const response = await openai.chat.completions.create({
      messages: [
        {
          role: "user",
          content: `Summarize the following proposal in 3 sentences: ${body}`,
        },
      ],
      model: "gpt-3.5-turbo",
    });
  ...
  useEffect(() => {
    ...
    if (cachedProposals) {
      ...
      setLoadingSummaries(true);
      (async () => {
        for (const proposal of JSON.parse(cachedProposals)) {
          proposal.summary = await getSummary(proposal.body);
        }
        setLoadingSummaries(false);
      })();
  ...
  return (
    <Flex flexDirection="column">
                ...
                <VStack paddingLeft="5px" align="start" width="100%">
                  <HStack alignContent="center" mb={2}>
                    <Badge
                      variant="subtle"
                      colorScheme={
                        proposal.state === "closed" ? "red" : "green"
                      }
                    >
                      {proposal.state === "closed" ? "Closed" : "Active"}
                    </Badge>
                    <Text color="white">
                      Autor: {proposal.author.slice(0, 6)}...
                  ...
                  {loadingSummaries ? (
                    ...
                    <Box minHeight="20px">
                      {" "}
                      {/* Altura mínima ajustada para garantir consistência */}
                      <Text
                        padding="5px"
                        color="aqua"
                        borderRadius="10px"
                        ...
                        <Button
                          ...
                          onClick={() => {
                            if (proposal.state === "closed") {
                              alert(
                                "Votação encerrada. Você está atrasado para votar!"
                              );
                            } else {
                              window.open(
      ...
      <ProposalModal
        ...
        proposalContent={modalContent}
        proposalTitle={modalTitle}
      />
    </Flex>
  );
};
