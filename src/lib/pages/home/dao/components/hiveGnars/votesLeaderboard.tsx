import { Box, Text, Image, Button, Link as ChakraLink } from "@chakra-ui/react";
...
const VotesLeaderboard: React.FC<votesLeaderboardProps> = ({ username }) => {
  ...
  const getVoteHistory = async () => {
    ...
    data.forEach((vote: any) => {
      ...
      const authorIndex = uniqueAuthors.findIndex(
        (author) => author.username === vote.author
      );
  ...
  const getLeaderboard = async () => {
    ...
    setUniqueAuthors((uniqueAuthors) =>
      uniqueAuthors.sort((a, b) => b.totalVoteRewards - a.totalVoteRewards)
    );
  ...
  useEffect(() => {
    if (username.length > 0) getLeaderboard();
  }, [username]);

  return (
    <Box w="100%" h="100%" borderRadius="xl" boxShadow="dark-lg" p="4">
        ...
        >
          ...
          {uniqueAuthors.slice(0, batchSize).map((author) => (
            ...
            >
              ...
              >
                <ChakraLink
                  href={`https://peakd.com/@${author.username}`}
                  isExternal
                >
                  @{author.username}
                </ChakraLink>
