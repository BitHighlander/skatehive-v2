...
const GnarsChart = () => {
  ...
  const fethRewardHistory = async (type: string) => {
    const response = await fetch(SQL_ENDPOINT, {
      ...
      body: JSON.stringify({
        query:
          type === "vote"
            ? VoteHistoryQuery("gnars")
            : CurrationHistoryQuery("gnars"),
      }),
    });
  ...
  const onStart = async () => {
    ...
    const total_vesting_fund_hive = parseFloat(
      props.total_vesting_fund_hive.toString().split(" ")[0]
    );
    const total_vesting_shares = parseFloat(
      props.total_vesting_shares.toString().split(" ")[0]
    );
    ...
    const ethValueFirstDay =
      ETH_INVESTMENT * priceHistoryETH.get(firstDay.toDateString());
    const hiveQuantityFirstDay =
      ethValueFirstDay / priceHistoryHIVE.get(firstDay.toDateString());

    // calculate daily reward earned
    const totalDailyReward =
      hivePower - totalCurationRewards - hiveQuantityFirstDay;
    ...
    priceHistoryHIVE.forEach((hivePrice: number, date: string) => {
      ...
      hiveQuantityWithVotes +=
        voteHistory.get(date) + curationHistory.get(date) + dailyReward;
