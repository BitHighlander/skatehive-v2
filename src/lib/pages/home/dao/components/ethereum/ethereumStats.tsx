...
const EthereumStats = () => {
  ...
  const [multisigETHBalance, setmultisigETHBalance] = useState<number | null>(
    null
  );
  ...
  const contract_gnars = new ethers.Contract(
    gnars_nftContract,
    ERC721_ABI,
    provider
  );
  ...
  async function readGnarsContract() {
    try {
      const result = await contract_gnars.getCurrentVotes(
  ...
  const usdWorthOfMultisigBalance =
    multisigETHBalance && ethPrice !== null
      ? `${(multisigETHBalance * ethPrice).toFixed(2)} USD`
      : "Loading...";
  ...
  const onStart = async () => {
    try {
      if (app) {
        ...
        const eth_hotwallet_call = await axios.get(
          "https://pioneers.dev/api/v1/portfolio/0xB4964e1ecA55Db36a94e8aeFfBFBAb48529a2f6c"
        );

        if (eth_hotwallet_call.status === 200) {
          const hotWalletBalanceAsNumber =
            eth_hotwallet_call.data.totalNetWorth;
  ...
  const totalWorthof2wallets =
    multisigETHBalance !== null && pioneerBalance !== null && ethPrice !== null
      ? `${(multisigETHBalance * ethPrice + parseFloat(pioneerBalance)).toFixed(
          2
        )} USD`
      : "Loading...";
      ...
      <VStack spacing={4} align="stretch">
        ...
        <Flex alignItems="center" justifyContent="center">
          <Text fontSize="2xl" fontWeight="bold" color="#7CC4FA">
            Total Worth:{" "}
            <Text style={{ fontSize: "48px" }}>{totalWorthof2wallets}</Text>
          </Text>
        </Flex>
        <HStack spacing={4} align="stretch">
          ...
          <BalanceDisplay
            ...
            label="Hot Wallet"
            balance={
              typeof pioneerBalance === "number"
                ? `${(pioneerBalance as number).toFixed(2)} USD`
                : pioneerBalance !== null && pioneerBalance !== undefined
                ? "Loading..."
                : ""
            }
          />
        ...
        <HStack spacing={4} align="stretch">
          <BalanceDisplay
            ...
            balance={
              usdWorthOfMultisigBalance !== null
                ? usdWorthOfMultisigBalance
                : "FUUUCK..."
            }
          />
          <BalanceDisplay
        ...
        >
          ...
          <Tooltip
            ...
            label="Voting Power of Skatehive Community on Gnars, tokens delegated by the community. Click to delegate."
          >
            <ChakraLink
              target="_blank"
        ...
        <Tooltip
          ...
          borderRadius="10px"
          border="1px dashed limegreen"
          label="Mint Page for Skatehive OG NFT. Click to Mint."
        >
          <HStack
            margin="10px"
            borderRadius="10px"
          ...
          >
            ...
            <ChakraLink
              target="_blank"
              href="https://zora.co/collect/eth:0x3ded025e441730e26ab28803353e4471669a3065/1"
              color="white"
              fontSize="16px"
            >
              Skatehive OG: 37
