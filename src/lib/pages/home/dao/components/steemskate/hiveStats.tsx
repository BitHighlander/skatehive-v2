...
const HiveStats: React.FC<HiveStatsProps> = ({ wallet }) => {
  ...
  const fetchHiveStats = async () => {
    try {
      ...
      const hiveWorth =
        parseFloat((account[0].balance as string).split(" ")[0]) *
        conversionRate;
      const hivePowerWorth =
        parseFloat(vestingSharesData.hivePower) * conversionRate;
      const delegatedHivePowerWorth =
        parseFloat(vestingSharesData.delegatedHivePower) * conversionRate;
      const hbdWorth =
        parseFloat((account[0].hbd_balance as string).split(" ")[0]) * hbdPrice;
      const savingsWorth =
        parseFloat((account[0].savings_hbd_balance as string).split(" ")[0]) *
        hbdPrice;

      const total =
        hiveWorth +
        hivePowerWorth +
        delegatedHivePowerWorth +
        hbdWorth +
        savingsWorth;
          ...
          <>
            <Flex alignItems="center" justifyContent="center">
              <Text fontSize="2xl" fontWeight="bold" color="red">
                Total Worth:{" "}
                <Text style={{ fontSize: "48px" }}>${total.toFixed(2)}</Text>
              </Text>
            </Flex>
            ...
            >
              ...
              >
                ...
                <ChakraLink
                  target="_blank"
                  href="https://vote.hive.uno/@skatehive"
                  fontSize="16px"
                >
                  Witness: 2.1M{" "}
                </ChakraLink>
            ...
            >
              ...
              >
                ...
                <ChakraLink
                  target="_blank"
                  href="https://hive.vote/dash.php?i=1&trail=steemskate"
                  fontSize="16px"
                >
                  Vote Trail: 37{" "}
                </ChakraLink>
