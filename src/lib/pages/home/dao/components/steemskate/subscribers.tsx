...
function SubscriberList() {
  ...
  const fetchSubscribersWithPagination = async (
    ...
    try {
      ...
      if (response.data && response.data.result) {
        const subscriberUsernames = response.data.result.map(
          (subscriber: any) => subscriber[0]
        );

        const subscriberInfoPromises = subscriberUsernames.map(
          async (username: any) => {
            try {
              ...
              const { last_post } = account[0];
              const { hbd_balance } = account[0];
              ...
              try {
                ...
                console.error(
                  `Error parsing JSON metadata for ${username}:`,
                  error
                );
              }

              const { vesting_shares } = account[0];
              ...
              const reputationResponse = await axios.post(bridgeApiUrl, {
                jsonrpc: "2.0",
              ...
              const reputation =
                reputationResponse.data.result[0]?.reputation || 0;
              return {
                username,
                authorAbout,
                hasVotedWitness,
              ...
              console.error(
                `Error fetching additional info for ${username}:`,
                error
              );
              return {
                username,
        ...
        const newSubscriberInfo = await Promise.all<any>(
          subscriberInfoPromises
        );
        setUserCount(
          (prevUserCount) => prevUserCount + subscriberUsernames.length
        );
  ...
  function isRecentlyActive(subscriberInfo: any) {
    ...
    const monthsDifference =
      (currentDate.getFullYear() - lastPostDate.getFullYear()) * 12 +
      (currentDate.getMonth() - lastPostDate.getMonth());
        ...
        {sortedSubscribers.map((subscriberInfo: SubscriberInfo) => (
          ...
          >
            <HStack justifyContent="center">
              <Avatar
                src={`https://images.ecency.com/webp/u/${encodeURIComponent(
                ...
                borderRadius="50%"
                border={
                  isRecentlyActive(subscriberInfo)
                    ? "5px solid limegreen"
                    : "5px solid red"
                } // Change border color based on activity
                marginBottom="10px"
              />

              <Link
                href={`https://skatehive.app/profile/${subscriberInfo.username}`}
                target="_blank"
              ...
              <Button
                variant="ghost"
                onClick={() => handleSendModalOpen(subscriberInfo.username)}
              >
              ...
              <Table variant="simple">
                <Tbody>
                  <Tr>
                    <Td>Witness</Td>
                    <Td>
                      {subscriberInfo.hasVotedWitness
                        ? "✅ Voted"
                        : "❌ Not Yet"}
                    </Td>
                  </Tr>
                  <Tr>
                    <Td>Reputation</Td>
                    <Td>
                      {calculateHumanReadableReputation(
                        subscriberInfo.reputation
                      )}
                    </Td>
                  </Tr>
                  <Tr>
                  ...
                  <Tr>
                    ...
                    <Td
                      color={
                        isRecentlyActive(subscriberInfo) ? "limegreen" : "red"
                      }
                    >
                      {formatLastUpdateDate(subscriberInfo.last_post)}
                    </Td>
                  </Tr>
                  <Tr>
                    <Td>Since</Td>
