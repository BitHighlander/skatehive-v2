...
const CommunityStats: React.FC<{ communityTag: string }> = ({
  communityTag,
}) => {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        // First API call
        const hiveHubResponse = await axios.get(
          `https://stats.hivehub.dev/communities?c=${communityTag}`
        );
        const hiveInfo = hiveHubResponse.data["hive-173115"];
        const totalHBDPayout =
          parseFloat(hiveInfo.total_payouts_hbd.replace("$", "")) || 0;
        ...
        const { items } = hiveBlogResponse.data.result;
        const skateHiveStats = items.find(
          (item: any) => item[1].toLowerCase() === "skatehive"
        );

        if (skateHiveStats) {
          const [, , totalPayout, numberOfPosts, numberOfAuthors] =
            skateHiveStats;
          setCommunityStats({
              ...
              <center>
                <HStack marginEnd="10px">
                  ...
                  >
                    ...
                    {communityStats.totalPayout.toFixed(2)}
                  </Text>
                  <Text
                    fontSize="8px"
                    marginLeft="-5px"
              ...
              <center>
                <HStack marginEnd="10px">
                  ...
                  <Text
                    ...
                    fontFamily="mono"
                  >
                    {communityStats.numberOfAuthors}
                  </Text>
                  <Text
                    fontSize="8px"
                    marginLeft="-5px"
