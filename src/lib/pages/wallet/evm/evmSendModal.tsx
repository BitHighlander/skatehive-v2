...
const EvmSendModal: React.FC<EvmSendModalProps> = ({
  isOpen,
  onClose,
  tokenInfo,
}) => {
  ...
  const onStart = async function () {
    try {
      ...
      const info = await api.SearchByNetworkId({ chainId });
      if (!info.data[0]) {
      ...
      const web3_provider_instance = new Web3(
        new Web3.providers.HttpProvider(info.data[0].service)
      );
  ...
  const handleSend = async () => {
    try {
      ...
      const amountDecimal = BigInt(
        Math.round(parseFloat(amount) * 10 ** prescision)
      );
