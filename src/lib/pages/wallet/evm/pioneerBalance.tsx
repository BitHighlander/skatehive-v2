...
const networkDetails = [
  {
    id: 1,
    name: "Ethereum",
    logo: "/assets/cryptologos/eth-logo.png",
    color: "blue.200",
  },
...
const PortfolioPage: React.FC<PortfolioPageProps> = ({ wallet_address }) => {
  ...
  const [totalBalanceUsdTokens, setTotalBalanceUsdTokens] = useState<
    number | null
  >(null);
  ...
  const [nftFormattedValue, setNftFormattedValue] = useState<number | null>(
    null
  );
  const [selectedToken, setSelectedToken] = useState<TokenInfo["token"] | null>(
    null
  );
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        const pioneer_ethereum_balance = await axios.get(
          `https://pioneers.dev/api/v1/getPubkeyBalance/ethereum/${wallet_address}`
        );
        ...
        const sortedTokens = response.data.tokens
          .map((token: TokenInfo) => token.token)
          .sort((a: any, b: any) => a.networkId - b.networkId);
            ...
            <Box marginBottom={2}>
              ...
              >
                <Text color="#FFA500" fontSize="18px" marginLeft="5px">
                  <Badge fontSize="24px">
                    {" "}
                    {ensAddress || formatWalletAddress(wallet_address)}
                  </Badge>
                </Text>
              </Button>
            </Box>
