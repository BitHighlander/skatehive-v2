    ...
    if (user) {
      try {
        ...
        const hivePowerWorth =
          (parseFloat(vestingSharesData.hivePower) +
            parseFloat(vestingSharesData.DelegatedToSomeoneHivePower)) *
          conversionRate;
        const hbdWorth = parseFloat(user.hbd_balance.split(" ")[0]) * hbdPrice;
        const delegatedToUserInUSD =
          parseFloat(vestingSharesData.delegatedToUserInUSD) * conversionRate;
        const savingsWorth =
          parseFloat(user.savings_hbd_balance.split(" ")[0]) * hbdPrice;
        const HPdelegatedToUser = parseFloat(
          vestingSharesData.HPdelegatedToUser
        );
        const total =
          hiveWorth +
          hivePowerWorth +
        ...
        setHivePower(
          `${vestingSharesData.DelegatedToSomeoneHivePower} (delegated to others)  + ${vestingSharesData.hivePower} (self delegated)`
        );
        setTotalWorth(total);
        setIsLoading(false);
        setOwnedTotal(total_Owned);
        setDelegatedToUserInUSD(
          `${delegatedToUserInUSD.toFixed(3).toString()} USD worth in HP`
        );
        setHPdelegatedToUser(
          `${HPdelegatedToUser.toFixed(3).toString()} HP delegated to you`
        );
      } catch (error) {
        ...
        <GridItem pl="1" area="main" padding="15px">
          {userLocation === "Brazil" ? (
            <Button
              ...
              onClick={() =>
                alert("Em breve! Por enquanto tente crowsnight.com")
              }
            >
          ...
          <Center>
            <VStack align="end">
              <Text fontSize="16px" fontWeight="bold" color="orange">
                You Own:{" "}
                <Badge borderRadius="10px" fontSize="20px" colorScheme="green">
                  {" "}
                  ${ownedTotal.toFixed(2)}
                </Badge>
              </Text>
              <Text fontSize="16px" fontWeight="bold" color="orange">
                Wallet Worth:{" "}
            ...
            <>
              ...
              >
                ...
                >
                  ...
                  <ChakraLink
                    target="_blank"
                    href="https://simpleswap.io/customer-account/signup?referral=DI8ePMnCsK"
                    fontSize="16px"
                  >
                    Buy HIVE{" "}
                  </ChakraLink>
              ...
              <Button
                ...
                justifyContent="center"
                padding="10px"
                onClick={handleOpenSendHBDModal}
              >
                SEND HBD
              </Button>
              <Button
              ...
              <Text fontSize="18px" fontWeight="bold" color="white">
                Hive Price:{" "}
                <Badge variant="outline" fontSize="24px" colorScheme="red">
                  {" "}
