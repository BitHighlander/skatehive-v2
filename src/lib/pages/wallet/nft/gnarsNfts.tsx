...
const GnarsNfts = () => {
    ...
    try {
      ...
      if (ETHaddress) {
        const portfolio = await axios.get(
          `https://pioneers.dev/api/v1/portfolio/${ETHaddress.toUpperCase()}`
        );
  ...
  const getGnarsFloorPrice = () => {
    const gnarsCollection = userPortfolios.find(
      (nft) => nft.token.collection.name === "Gnars"
    );
  ...
  const getAllGnarsEstimatedValue = () => {
    const gnarsNfts = userPortfolios.filter(
      (nft) => nft.token.collection.name === "Gnars"
    );
    const totalEstimatedValue = gnarsNfts.reduce(
      (total, nft) => total + +nft.token.estimatedValueEth,
      0
    );
        ...
        >
          ...
          >
            <GridItem pl="2" bg="orange.300" area="header" borderRadius="10px">
              <Center>
                <Text fontFamily="Londrina Solid" color="black" fontSize="32px">
                  Gnars Dao
                </Text>
              </Center>
            </GridItem>
              ...
              .map((nft, index) => (
                ...
                >
                  <VStack alignItems="start" spacing={1} flex="1">
                    ...
                    >
                      Estimated Value:{" "}
                      {(+nft.token.estimatedValueEth).toFixed(3)} ETH
                    </Text>
                  </VStack>
                </Flex>
              ))}
