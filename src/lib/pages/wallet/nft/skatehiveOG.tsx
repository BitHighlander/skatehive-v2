...
const SkatehiveOG = ({ wallet }: { wallet: string }) => {
  ...
  const [userGnarsBalance, setUserGnarsBalance] = useState<string | undefined>(
    undefined
  );
  ...
  useEffect(() => {
    async function fetchWalletData() {
      try {
        ...
        const gnarsContract = new ethers.Contract(
          gnars_contract,
          ERC721_ABI,
          provider
        );
        ...
        const skatehiveOGbalance = await skthvProxyContract.balanceOf(
          wallet,
          1
        );
        const readableOGBalance = ethers.utils.formatUnits(
          skatehiveOGbalance,
          0
        );
        setUserVotes(
          50 * parseFloat(readableOGBalance) + parseFloat(readableGnarsBalance)
        );
  ...
  const handleBuyButtonClick = () => {
    ...
    window.location.href =
      "https://zora.co/collect/eth:0x3ded025e441730e26ab28803353e4471669a3065/1";
  };
        ...
        {userBalance === "0" ? (
          ...
          >
            ...
            <Text fontSize="sm" fontWeight="bold" color="gray.500" mb="2">
              100% of earnings from NFT sales will be held in Skatehive Vault,
              and the community will decide how and where the money will be
              spent via proposals on Skatehive Snapshot.
            </Text>
          </Flex>
        ) : (
          <Grid
