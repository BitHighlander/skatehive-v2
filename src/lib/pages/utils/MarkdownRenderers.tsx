...
export const MarkdownRenderers = {
  ...
  p: ({ children, ...props }: RendererProps) => (
    <div
      {...props}
      style={{ color: "white", fontSize: "18px", paddingBottom: "15px" }}
    >
      {children}
    </div>
  ),

  a: ({ children, ...props }: RendererProps) => (
    <a {...props} style={{ color: "orange", margin: "0px" }}>
      {children}
    </a>
  ),
  ...
  ul: ({ ordered, children, ...props }: RendererProps) => {
    const listType = ordered ? "1" : "decimal";
    return (
      <ul {...props} data-ordered={listType} style={{ paddingLeft: "10%" }}>
        {children}
      </ul>
    );
  },
  ...
  hr: ({ children, ...props }: RendererProps) => (
    <hr {...props} style={{ paddingBottom: "20px" }}>
      {children}
    </hr>
  ),
  ...
  strong: ({ children, ...props }: RendererProps) => (
    <strong {...props} style={{ color: "orange" }}>
      {children}
    </strong>
  ),
};
