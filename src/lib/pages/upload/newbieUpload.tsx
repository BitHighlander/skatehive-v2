...
const IntroductionPost: React.FC = () => {
    ...
    try {
      if (file.type.startsWith("video/mp4")) {
        ...
        if (response.ok) {
          ...
          console.error(
            "Error uploading video file to Pinata IPFS:",
            errorData
          );
        }
        ...
        if (response.ok) {
          ...
          const errorData = await response.json();
          console.error(
            "Error uploading image file to Pinata IPFS:",
            errorData
          );
        }
      } else {
  ...
  const onDropImages = async (acceptedFiles: File[]) => {
    ...
    if (acceptedFiles[0].type.startsWith("video/")) {
      ...
      if (isVideoUploaded) {
        alert(
          "You can only upload one video per post. Video will be uploaded on 3Speak."
        );
        setIsUploading(false);
        return;
      }
  ...
  const handleHiveUpload = async () => {
    ...
    if (isVideoUploaded) {
      if (!videoThumbnailUrl) {
        alert(
          "Please upload video thumbnail on 3Speak first. You can do this by clicking the 'Set Video Thumbnail' after sleeking to any point in the video button."
        );
        return;
      }
    }
    ...
    if (isVideoUploaded) {
      ...
      const newThumbnailURL = `${ipfs3Speak}${videoInstance.thumbnail.replace(
        "ipfs://",
        ""
      )}/`;
      ...
      const updatedVideoInstance = await setVideoInfoOn3Speak(
        updateObject,
        true
      ); // true means update
      ...
      const encoderBeneficiaries = JSON.parse(
        updatedVideoInstance.beneficiaries
      );
      const encoderBeneficiary = encoderBeneficiaries.find(
        (b: any) => b.src === "ENCODER_PAY"
      );
    ...
    if (user && title) {
      ...
      if (username) {
        ...
        finalBeneficiaries = finalBeneficiaries.sort((a: any, b: any) =>
        ...
        const commentOptions = {
          author: username,
          permlink: videoPermlink || permlink, // Use the video permlink if video is uploaded on 3Speak
        ...
        if (includeFooter) {
          const finalPermlink = videoPermlink ? videoPermlink : permlink;
          ...
          finalMarkdown += newFooter;
          setMarkdownText((prevMarkdown) => prevMarkdown + newFooter);
        }
        const newTitle = title + " - My introduction post to Skatehive";
        ...
        window.hive_keychain.requestBroadcast(
          ...
          async (response: any) => {
            if (response.success) {
              ...
              if (isVideoUploaded) {
                window.alert(
                  "Video successfully published on 3Speak! It will be available soon!"
                );
              }
            } else {
  ...
  const renderTags = () => {
    return tags.map((tag, index) => (
      ...
      >
        ...
        >
          <Badge colorScheme="teal" variant="subtle" marginTop={"10px"}>
            {tag}
          </Badge>
          <CloseButton
            size="xs"
  ...
  const handleAuthorSearch = (searchUsername: string) => {
    ...
    if (!beneficiaryExists && percentage > 0) {
      ...
      alert(
        `Beneficiary ${searchUsername} added! Please set the percentage using the sliders below.`
      );
    } else {
      alert(
        `Beneficiary ${searchUsername} already exists or percentage is zero.`
      );
    }
  ...
  const handleCountryChange = (event: React.ChangeEvent<HTMLSelectElement>) => {
    ...
    const selectedCountryObject = countryOptions.find(
      (country) => country.value === selectedOption
    );
      ...
      <Box mt={0}>
        <VStack>
          ...
          <Badge color="black" bg="yellow" marginTop="15px">
            Before start posting, tell the community a little a few things about
            you{" "}
          </Badge>
        </VStack>
      </Box>
      <Flex minWidth="100%" flexDirection={isMobile ? "column" : "row"}>
        <Box
          ...
          <Flex flexDirection={isMobile ? "column" : "row"}>
            <Box flex={1}>
              ...
              >
                ...
                <Text>
                  {" "}
                  Drop images{isVideoUploaded ? "" : "/video"} or click to
                  select
                </Text>
              </VStack>
              <Flex
              ...
              >
                <Checkbox
                  isDisabled={videoFile || uploadedVideo ? true : false}
                  isChecked={is3speakPost}
                  onChange={() => setIs3speakPost(!is3speakPost)}
                >
        ...
        >
          <Box>
            ...
            >
              ...
              <Text marginLeft="10px" fontSize="36px" fontWeight="bold">
                {" "}
                |{" "}
              </Text>
              <Text
