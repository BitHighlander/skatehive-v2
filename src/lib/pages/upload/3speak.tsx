...
export const get3SpeakAuthStatus = (): AuthStatus => {
  ...
  const connected = !!localStorage.getItem("3SpeakToken");
...
export const Connect3Speak = () => {
    ...
    window.hive_keychain.requestVerifyKey(
      ...
      async (response: any) => {
        if (response.success) {
          ...
          alert(
            "Something went wrong while connecting to 3Speak. Please try again."
          );
          removeToken();
        }
      }
  ...
  const onStart = () => {
    ...
    if (
      localStorage.getItem("3SpeakUser") === user.name &&
      localStorage.getItem("3SpeakToken")
    ) {
      setConnected(true);
    } else {
      setConnected(false);
  ...
  return (
    <Box>
      <Button
        onClick={connect}
        colorScheme={connected ? "green" : "teal"}
        size="sm"
      >
...
export const uploadTo3Speak = async (
  file: File,
  setIsUploading: Function,
  setVideoUploadProgress: Function,
  setVideoInfo: Function,
  setIsVideoUploaded: Function
) => {
...
export const uploadThumbnailTo3Speak = (
  file: File,
  setIsUploading: Function,
  videoInfo: {},
  setVideoInfo: Function,
  setVideoThumbnailUrl: Function,
...
export const setVideoInfoOn3Speak = async (videoInfo: {}, isUpdate = false) => {
  const SET_VIDEO_INFO_URL = `https://studio.3speak.tv/mobile/api/${
    isUpdate ? "update_info" : "upload_info?app=skatehive"
  }`;
