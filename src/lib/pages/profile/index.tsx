...
export default function ProfilePage() {
    ...
    const fetchUserInfo = async () => {
      ...
      try {
        ...
        const metadata = JSON.parse(
          user_account[0].posting_json_metadata || ""
        );
        ...
        const { about } = jsonMetadata.profile;
        const ethAddress = jsonMetadata.extensions?.eth_address;
        const { location } = jsonMetadata.profile;
        const displayname = jsonMetadata.profile.name;
        const { created } = jsonMetadata.profile;
  ...
  const UserAbout = ({ user }: any) => {
    if (user) {
      try {
        return (
          <Box border="1px solid " p={8} borderRadius={20} boxShadow="lg">
