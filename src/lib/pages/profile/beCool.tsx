...
export default function AuthorProfilePage() {
  ...
  const [delegated_vesting_shares, setDelegatedVestingShares] =
    useState<string>("");
  const [received_vesting_shares, setReceivedVestingShares] =
    useState<string>("");
  const [hivePower, setHivePower] = useState<string>("");
  const [hasFollowCurationTrail, setHasFollowCurationTrail] =
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        ...
        setDelegatedVestingShares(
          String(account[0]?.delegated_vesting_shares || "")
        );
        setReceivedVestingShares(
  ...
  const handleWitnessVote = async () => {
    try {
      ...
      const formParamsAsObject = {
        data: {
          username, // Use the current username
        ...
        <GridItem>
          <VStack>
            ...
            <Badge
              ...
              p="10px"
            >
              HivePower: {hivePower} HP{" "}
              {hasMoreThan500HP ? "✅" : "Insufficient"}
            </Badge>
            {!hasMoreThan500HP && (
