...
import type { Custom } from "keychain-sdk";
...
const DEFAULT_COVER_IMAGE_URL =
  "https://i.ibb.co/r20bWsF/You-forgot-to-add-a-cover.gif";
...
export default function AuthorProfilePage() {
    ...
    const fetchAccountInfo = async () => {
      if (username) {
        ...
        try {
          ...
          const author_account = await client.database.getAccounts([
            authorName,
          ]);
          ...
          const hbd_balance = parseFloat(
            author_account[0].hbd_balance.toString()
          );
          const vesting_shares = parseFloat(
            author_account[0].vesting_shares.toString()
          );
          ...
          const metadata = JSON.parse(
            author_account[0].posting_json_metadata
          ) as Author;
          const authorAbout = metadata.profile?.about || null;
          const coverImage =
            metadata.profile?.cover_image || DEFAULT_COVER_IMAGE_URL;
          setCoverImageUrl(coverImage);
          setAuthorAbout(authorAbout);
          setProfile_image(
            metadata.profile?.profile_image || DEFAULT_COVER_IMAGE_URL
          );
  ...
  useEffect(() => {
    if (user && authorName) {
      getRelationshipBetweenAccounts(user?.user?.name, authorName).then(
        (result) => {
  ...
  return (
    ...
    >
      ...
      >
        ...
        {profile_image.includes(
          "storage.googleapis.com/zapper-fi-assets/nfts"
        ) && (
          <Image
        ...
        >
          ...
          >
            <TabList color="white" justifyContent="center">
              <Tab borderRadius="10px" border="1px solid ">
                Posts
              </Tab>
              <Tab borderRadius="10px" border="1px solid ">
                About
