...
import type { Broadcast } from "keychain-sdk";
...
const EditProfileModal: React.FC<EditProfileModalProps> = ({
  ...
  useEffect(() => {
    if (user && user.posting_json_metadata) {
      try {
        ...
        setAvatarUrl(
          (avatarUrl) => avatarUrl || metadata.profile.profile_image || ""
        );
        setCoverImageUrl(
          (coverImageUrl) => coverImageUrl || metadata.profile.cover_image || ""
        );
  ...
  const handleProfileFileInputChange = (
    ...
    if (file) {
      ...
      uploadFileToIPFS(file).then((ipfsUrl) =>
        setAvatarUrl(ipfsUrl?.toString() || "")
      );
    }
  };

  const handleCoverFileInputChange = (
    e: React.ChangeEvent<HTMLInputElement>
  ) => {
    const file = e.target.files?.[0];
    if (file) {
      ...
      uploadFileToIPFS(file).then((ipfsUrl) =>
        setCoverImageUrl(ipfsUrl?.toString() || "")
      );
    }
  };
      ...
      <ModalContent bg="black" border="1px solid limegreen">
        ...
        <ModalBody>
          ...
          <HStack>
            ...
            {selectedProfileFile && <div>{selectedProfileFile.name}</div>}
          </HStack>
          ...
          {selectedCoverFile && <div>{selectedCoverFile.name}</div>}
          <Text> Website</Text>
          <Input
            placeholder="Website"
            value={website}
        ...
        <Flex align="center" justify="center" direction="column">
          <Button onClick={() => setIsEthSetupModalOpen(true)}>
            {" "}
