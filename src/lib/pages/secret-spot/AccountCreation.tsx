...
import type { KeyRole, Operation } from "@hiveio/dhive";
import {
  KeychainSDK,
  KeychainKeyTypes,
...
import { FaCheck, FaTimes, FaKey, FaCopy, FaDownload } from "react-icons/fa";
...
const generatePassword = () => {
  ...
  const key = `SKATE000${dhive.PrivateKey.fromSeed(
    array.toString()
  ).toString()}`;
  ...
  const handleCheck = async () => {
    if (desiredUsername) {
      ...
      if (isAvailable) {
        ...
        console.log(
          "Account already exists. Please choose a different desiredUsername."
        );
            ...
            >
              ...
              >
                <Button
                  ...
                  colorScheme="yellow"
                  border="2px solid black"
                  onClick={() => copyToClipboard(downloadText)}
                >
                  Copy Keys
                </Button>
                <Button
