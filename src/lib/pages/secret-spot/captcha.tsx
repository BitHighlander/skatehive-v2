...
const Captcha: React.FC<CaptchaProps> = ({ onCaptchaCompletion }) => {
  ...
  const [userImagePositions, setUserImagePositions] = useState<
    Record<string, ImageItem | null>
  >({
    A1: null,
    A2: null,
    A3: null,
  ...
  const handleDrop = (imageId: string, position: string) => {
    ...
    setUserAnswers([
      ...userAnswers,
      `User's Choice for ${position}: ${imageId}`,
    ]);

    if (image) {
      setOccupiedPositions(
        (prevOccupiedPositions) => prevOccupiedPositions + 1
      );
    }
  };
  ...
  return (
    <Box>
      <Center>
        <img src="https://i.ibb.co/6ZqHYdK/image.png" />
      </Center>
...
const DroppedImage: React.FC<ImageItem> = ({ id, src }) => {
  return (
    <Box w="160px" h="160px" position="relative" overflow="hidden">
      <Image
        src={src}
        alt={`Image ${id}`}
        w="auto"
...
const DraggableImage: React.FC<ImageItem> = ({ id, src }) => {
  ...
  return (
    <Box ref={ref} cursor="pointer" w="80px" h="80px" marginTop="50px">
      <Image src={src} alt={`Image ${id}`} w="80px" h="80px" />
    </Box>
  );
};
  ...
  for (const position in userImagePositions) {
    if (userImagePositions.hasOwnProperty(position)) {
      const isPositionCorrect =
        isAnswerCorrect && userImagePositions[position]
          ? trickCorrectAnswers[randomTrickName][position].includes(
              userImagePositions[position]!.id
            )
          : false;
