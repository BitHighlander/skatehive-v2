...
import {
  fetchConversionRate,
  fetchHbdPrice,
} from "lib/pages/utils/apis/coinGecko";
...
const HeaderNew = () => {
  ...
  const [totalNetWorth, setTotalNetWorth] = useState<number | null>(0.0);
  ...
  const NotificationModal: React.FC<NotificationModalProps> = ({
    ...
    const extractMsgDetails = (msg: string) => {
      ...
      return match
        ? { author: match[1], text: match[2] }
        : { author: "", text: msg };
    };

    const [avatarSrcMap, setAvatarSrcMap] = useState<
    ...
    const calculateTimeAgo = (timestamp: string) => {
      ...
      const notificationTimeLocal = new Date(
        notificationTime.getTime() - userTimeZoneOffset * 60 * 1000
      );

      const timeDifference =
        new Date().getTime() - notificationTimeLocal.getTime();
      const minutesAgo = Math.floor(timeDifference / (1000 * 60));

      // If the notification time is more than 24 hours ago, show the full date
      if (minutesAgo > 24 * 60) {
        return notificationTimeLocal.toLocaleString("en-US", {
          dateStyle: "medium",
          timeStyle: "short",
        });
      }
      ...
      return `${Math.abs(minutesAgo)} ${
        Math.abs(minutesAgo) === 1 ? "minute" : "minutes"
      } ago`;
    };
    ...
    useEffect(() => {
      ...
      const uniqueAuthors = Array.from(
        new Set(
          notifications.map(
            (notification) => extractMsgDetails(notification.msg).author
          )
        )
      );
        ...
        >
          ...
          <PopoverBody overflow="auto">
            {notifications.map((notification) => {
              const { author } = extractMsgDetails(notification.msg);
              ...
              return (
                ...
                >
                  <HStack spacing={4}>
                    <Box boxSize="56px">
                      <Link
                        to={`https://skatehive.app/profile/${author}`}
                    ...
                    <Text fontSize="sm">
                      {extractMsgDetails(notification.msg).text}
                    </Text>
  ...
  const onLoad = async function () {
    try {
      if (
        ...
        app.wallets[0].wallet &&
        app.wallets[0].wallet.accounts
      ) {
    ...
    const fetchData = async () => {
      try {
        const response = await axios.get(
          `https://pioneers.dev/api/v1/portfolio/${wallet_address?.toUpperCase()}`
        );
        ...
        if (response.data !== undefined) {
          ...
          if (response.data.nfts && Array.isArray(response.data.nfts)) {
            const gnarsNFTsCount = response.data.nfts.reduce(
              (count: any, nft: any) => {
                if (
                  nft.token &&
  ...
  const avatarUrl =
    user && user.posting_json_metadata !== ""
  ...
  const convertVestingSharesToHivePower = async (
    ...
    const delegatedVestingSharesFloat = parseFloat(
      delegatedVestingShares.split(" ")[0]
    );
    const receivedVestingSharesFloat = parseFloat(
    ...
    const HPdelegatedToOthers =
      (parseFloat(result.result.total_vesting_fund_hive) *
        delegatedVestingSharesFloat) /
      parseFloat(result.result.total_vesting_shares);
    return {
  ...
  const onStart = async function (
    ...
    if (user) {
      try {
        ...
        const hiveWorth =
          parseFloat(user.balance.split(" ")[0]) * conversionRate;

        const hivePowerWorth =
          (parseFloat(vestingSharesData.availableHivePower) +
            parseFloat(vestingSharesData.HPdelegatedToOthers)) *
          conversionRate;

        const hbdWorth = parseFloat(user.hbd_balance.split(" ")[0]) * hbdPrice;
        const savingsWorth =
          parseFloat(user.savings_hbd_balance.split(" ")[0]) * hbdPrice;
        ...
        setHivePowerText(
          `${vestingSharesData.availableHivePower} + ${vestingSharesData.HPdelegatedToOthers} (delegated)`
        );
    ...
    >
      ...
      >
        <Menu>
          ...
          >
            <Link
              to="https://snapshot.org/#/skatehive.eth"
              target="_blank"
              style={{ textDecoration: "none" }}
            >
              <MenuItem
            ...
            <Link
              to="https://hive.vote/dash.php?i=1&trail=steemskate"
              target="_blank"
              style={{ textDecoration: "none" }}
            >
              <MenuItem
            ...
            <Link
              to="https:/github.com/sktbrd/skateapp"
              target="_blank"
              style={{ textDecoration: "none" }}
            >
              <MenuItem
        ...
        <Box>
          <HStack spacing={3} alignItems="center">
            <ChakraLink as={RouterLink} to="/wallet">
              {isDesktop && (
                <Tooltip
                  label="Total Networth counting tokens + NFT Value"
      ...
      <Flex
        gap={{ base: 4, md: 8 }}
        padding={{ base: "6px 18px", md: "8px 20px" }}
        borderRadius="6px"
        position={{ md: "absolute" }}
        border="2px solid limegreen"
      >
        <Button variant="link" color="white" as={Link} to="/">
          Home
        </Button>
        <Button variant="link" color="white" as={Link} to="/QFS">
          Play
        </Button>
        {
          // Se não tiver logado
